 ...
 * @see midi.toMidi
 */
var midi$1 = function (note) { return props(note).midi || +note || null; };
 ...
 * Note.freq(69) // => 440
 */
var freq$1 = function (note) { return props(note).freq || midiToFreq(note); };
...
var encode = function (ref) {
  ...
  var o = oct$$1 - FIFTH_OCTS[step] - 4 * alt;
  return [dir * f, dir * o];
};
...
var decode = function (f, o, dir) {
  ...
  var oct$$1 = o + 4 * alt + FIFTH_OCTS[step];
  return { step: step, alt: alt, oct: oct$$1, dir: dir };
};
...
var encoder = function (props$$1) { return memo$1(function (str) {
    var p = props$$1(str);
 ...
 */
function transpose$1(note, interval) {
  if (arguments.length === 1) { return function (i) { return transpose$1(note, i); }; }
 ...
 */
function fifths(from$$1, to) {
  if (arguments.length === 1) { return function (to) { return fifths(from$$1, to); }; }
  var f = encodeNote(from$$1);
  var t = encodeNote(to);
...
function transposeBy(interval, note) {
  if (arguments.length === 1) { return function (n) { return transpose$1(n, interval); }; }
  return transpose$1(note, interval);
}
 ...
 */
function interval$1(from$$1, to) {
  if (arguments.length === 1) { return function (t) { return interval$1(from$$1, t); }; }
  var f = encodeNote(from$$1);
  var t = encodeNote(to);
 ...
 */
function semitones$1(from$$1, to) {
  if (arguments.length === 1) { return function (t) { return semitones$1(from$$1, t); }; }
  var f = props(from$$1);
  var t = props(to);
...
var sdata = {
	...
	"dorian pentatonic": ["1P 3m 4P 5P 7m"],
	"phrygian pentatonic": ["1P 3m 4P 5P 7m"],
...
var m7 = ["1P 3m 5P 7m",["minor7","_","_7","min7"]];
...
var mMaj7 = ["1P 3m 5P 7M",["mM7","minMaj7","mMaj7","_M7"]];
var mMaj7b6 = ["1P 3m 5P 6m 7M",["mM7b6","minMaj7b6"]];
var mM9 = ["1P 3m 5P 7M 9M",["mMaj9","minMaj9","-M9"]];
var mM9b6 = ["1P 3m 5P 6m 7M 9M",["mMaj9b6","minMaj9b6"]];
...
var cdata = {
	...
	"64": ["5P 8P 10M"],
	"M#5": ["1P 3M 5A",["augmented","maj#5","+","aug","Maj#5"]],
	"M#5add9": ["1P 3M 5A 9M",["+add9"]],
	"M13#11": ["1P 3M 5P 7M 9M 11A 13M",["maj13#11","maj13+4","M13+4","maj13#4","M13#4","Maj13#11","Maj13+4","Maj13#4"]],
	"M6#11": ["1P 3M 5P 6M 11A",["maj6b5","M6b5","6b5","maj6#11","6#11","Maj6b5","Maj6#11"]],
	"M69#11": ["1P 3M 5P 6M 9M 11A"],
	"M7#11": ["1P 3M 5P 7M 11A",["maj7#11","maj7+4","M7#4","Maj7#11","Maj7+4"]],
	"M7#5": ["1P 3M 5A 7M",["maj7#5","M7#5","maj7+","M7+","Maj7#5","Maj7+"]],
	"M7#5sus4": ["1P 4P 5A 7M"],
	"M7#9#11": ["1P 3M 5P 7M 9A 11A"],
	"M9#11": ["1P 3M 5P 7M 9M 11A",["maj9#11","maj9+4","M9+4","maj9#4","M9#4","Maj9#11","Maj9+4","Maj9#4"]],
	"M9#5": ["1P 3M 5A 7M 9M",["maj9#5","Maj9#5"]],
 ...
 * chord.names(); // => ["Maj3", ...]
 */
var chord$1 = dictionary(cdata);
var pcset = combine(scale$1, chord$1);
var allScales = sdata;
var allChords = cdata;


var es6$4 = Object.freeze({
...
var properties$2 = function (name$$1) {
  var intervals$$1 = scale$1(name$$1);
  if (!intervals$$1) { return NO_SCALE; }
  var s = { intervals: intervals$$1, name: name$$1 };
  s.chroma = chroma$2(intervals$$1);
 ...
 */
var intervals$1 = function (name$$1) {
...
function notes(nameOrTonic, name$$1) {
  ...
  name$$1 = name$$1 || p[1];
  return intervals$1(name$$1).map(transpose$1(p[0]));
}
...
var modeNames = function (name$$1) {
  var ivls = intervals$1(name$$1);
  var tonics = notes(name$$1);

  return modes(ivls)
    .map(function (chroma$$1, i) {
      var name$$1 = scale$1.names(chroma$$1)[0];
...
var chords = function (name$$1) {
  var inScale = isSubsetOf(intervals$1(name$$1));
  return chord$1.names().filter(function (name$$1) { return inScale(chord$1(name$$1)); });
};
...
var toScale = function (notes) {
  var pcset$$1 = compact(notes.map(pc));
...
var supersets = function (name$$1) {
  if (!intervals$1(name$$1).length) { return []; }
  var isSuperset = isSupersetOf(intervals$1(name$$1));
  return scale$1.names().filter(function (name$$1) { return isSuperset(scale$1(name$$1)); });
};
...
var subsets = function (name$$1) {
  var isSubset = isSubsetOf(intervals$1(name$$1));
  return scale$1.names().filter(function (name$$1) { return isSubset(scale$1(name$$1)); });
};


var es6$6 = Object.freeze({
	props: props$2,
	names: names$2,
...
var properties$3 = function (name$$1) {
  var intervals$$1 = chord$1(name$$1);
  if (!intervals$$1) { return NO_CHORD; }
  var s = { intervals: intervals$$1, name: name$$1 };
  s.chroma = chroma$2(intervals$$1);
  s.setnum = parseInt(s.chroma, 2);
  s.names = chord$1.names(s.chroma);
  return s;
};
 ...
 */
var intervals$2 = function (name$$1) { return props$3(tokenize$3(name$$1)[1]).intervals; };
...
function notes$1(nameOrTonic, name$$1) {
  ...
  name$$1 = name$$1 || p[1];
  return props$3(name$$1).intervals.map(transpose$1(p[0]));
}
 ...
 */
var exists$1 = function (name$$1) { return chord$1(tokenize$3(name$$1)[1]) !== undefined; };
 ...
 */
var supersets$1 = function (name$$1) {
  if (!intervals$2(name$$1).length) { return []; }
  var isSuperset = isSupersetOf(intervals$2(name$$1));
  return chord$1.names().filter(function (name$$1) { return isSuperset(chord$1(name$$1)); });
};
 ...
 * @return {Array} a list of chord names
 */
var subsets$1 = function (name$$1) {
  var isSubset = isSubsetOf(intervals$2(name$$1));
  return chord$1.names().filter(function (name$$1) { return isSubset(chord$1(name$$1)); });
};
 ...
 */
function tokenize$3(name$$1) {
  var p = tokenize(name$$1);
  if (p[0] === "") { return ["", name$$1]; }
...
exports.Array = es6;
exports.Note = es6$1;
exports.Interval = es6$2;
exports.Distance = es6$3;
exports.Scale = es6$6;
exports.Chord = es6$7;
exports.PcSet = es6$5;
exports.Dictionary = es6$4;
